...
//   /**
//    * [핵심 수정] 다인 논쟁 대응 Side 결정 로직
//    * 1. 참여자 목록(participants)에서 현재 유저의 인덱스를 찾습니다.
//    * 2. 인덱스 0은 'A', 1은 'B', 2는 'C'... 순으로 할당됩니다.
//    */
//   const getMySideInfo = () => {
//     if (!debate || !user) return { side: 'A', label: 'A측' };
    
//     // API 응답에 participants 배열이 있다고 가정 (없을 경우 creator_id로 폴백)
//     const participants = debate.participants || [];
//     const myIndex = participants.findIndex(p => String(p.user_id || p.id) === String(user.id));
    
//     // 만약 참여자 명단에 없다면 (예외 상황) 개설자 여부로 판단
//     if (myIndex === -1) {
//       const isCreator = String(debate.creator_id) === String(user.id);
//       return isCreator ? { side: 'A', label: 'A측' } : { side: 'B', label: 'B측' };
//     }

//     const sideCode = String.fromCharCode(65 + myIndex); // 0->A, 1->B...
//     return {
//       side: sideCode,
//       label: `${sideCode}측`
//     };
...
//   const { side, label } = getMySideInfo();
...
//     try {
//       // 계산된 동적 side('A', 'B', 'C' 등)를 전송
//       await submitArgument(debateId, { content, side })
//       // 제출 후 판결 대기/진행 페이지로 이동
//       navigate(`/debate/${debateId}/judging`)
//     } catch (err) {
//       alert(err.message || '제출에 실패했습니다.')
//     }
...
//   if (loading) return (
//     <div className="min-h-screen bg-[#FAFAF5] flex items-center justify-center">
//       <div className="animate-pulse text-gray-400 font-medium tracking-widest uppercase">Analyzing Debate...</div>
//     </div>
//   )
...
//             🔍 {debate?.lens}
//           </span>
//           {/* 다인 논쟁인 경우 인원수 표시 배지 추가 (옵션) */}
//           {debate?.participants?.length > 2 && (
//             <span className="bg-amber-400/20 border border-amber-400/30 px-3 py-1.5 rounded-full text-[11px] font-bold text-amber-200 tracking-tight backdrop-blur-md">
//               👥 {debate.participants.length}인 논쟁
//             </span>
//           )}
//         </div>
...
//         {/* 진영 안내: 동적으로 변하는 label 적용 */}
//         <div className="flex justify-between items-end px-1">
//           <p className="text-gray-500 text-[13px] font-bold">
//             당신의 논리적인 주장 <span className="text-[#1a2744] text-sm">({label})</span>
//           </p>
//           <span className="text-[11px] text-gray-400 italic font-medium">공백 포함 50자 이상</span>
//         </div>
...
//           <textarea
//             className="w-full h-72 p-6 focus:outline-none resize-none text-[16px] leading-[1.6] text-gray-800 placeholder:text-gray-200"
//             placeholder={`${label}로서 상대방을 설득할 수 있는 강력한 근거를 제시해주세요. 논리적인 흐름이 AI 판결에 큰 영향을 미칩니다.`}
//             value={content}
//             onChange={(e) => setContent(e.target.value)}
//             maxLength={MAX_CHAR}
//           />
...
//               : 'bg-[#1a2744] text-white shadow-[#1a2744]/20 hover:bg-[#151f36] cursor-pointer'
//           }`}
//         >
//           주장 제출하기
//         </button>
...
//         <p className="text-center text-gray-300 text-[11px] font-medium leading-relaxed px-4">
//           제출된 주장은 수정할 수 없으며,<br/>
//           {debate?.participants?.length > 2 ? `모든 참여자의 주장이 모이면 ` : '상대방의 주장이 완료되면 '} 
//           AI 판사의 분석이 시작됩니다.
//         </p>
//       </form>
//     </div>
