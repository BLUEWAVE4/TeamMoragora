...
export default function ArgumentPage() {
 ...
 try {
    // 이미 정의한 isCreator를 사용하세요
    const side = isCreator ? 'A측' : 'B측' 
  ...
  <form onSubmit={handleSubmit} className="w-full max-w-md flex flex-col p-6 pt-14 gap-5">
      ...
      <div className="px-6 py-3.5 border-t border-gray-50 flex justify-between items-center bg-gray-50/60">
        <div className="flex items-center gap-1.5">
          <div className={`w-1.5 h-1.5 rounded-full ${
            content.trim().length === 0 ? 'bg-gray-200' : 'bg-emerald-500'
          }`} />
          <span className={`text-[11px] font-bold ${
            content.trim().length === 0 ? 'text-gray-300' : 'text-emerald-600'
          }`}>
            {content.trim().length === 0 ? '주장을 입력해주세요' : '제출 준비 완료 ✓'}
          </span>
        </div>
        <div className="text-[13px] tabular-nums">
          <span className={`font-black ${content.trim().length === 0 ? 'text-gray-300' : 'text-[#1B2A4A]'}`}>
    ...
    >
      {isSubmitting ? (
        ...
        <>
          <span>주장 제출하기</span>
          {content.trim().length > 0 && <span className="text-[#D4AF37]/60 text-[14px]">→</span>}
        </>
      )}
    </button>
