...
export default function ArgumentPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // debate 데이터가 있을 때만 creator_id 확인
        ...
        {(debate?.pro_side || debate?.con_side) && (
          <div className="mt-5 space-y-2">
            <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 rounded-xl px-4 py-2.5">
              <span className="text-[11px] font-black text-emerald-400 shrink-0">찬성</span>
            ...
            <div className="flex items-center gap-2 bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-2.5">
              <span className="text-[11px] font-black text-red-400 shrink-0">반대</span>
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-md flex flex-col p-6 pt-12 gap-5">
        ...
        <div className="flex justify-between items-center px-1">
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${sideLabel === '찬성' ? 'bg-emerald-500' : 'bg-red-500'}`} />
            <p className="text-gray-600 text-[13px] font-bold">
              {nickname ? `${nickname}의 주장` : '당신의 주장'}
            </p>
            <span className={`text-[11px] font-black px-2 py-0.5 rounded-full ${sideLabel === '찬성' ? 'bg-emerald-50 text-emerald-600 border border-emerald-200' : 'bg-red-50 text-red-500 border border-red-200'}`}>
              {sideLabel}
            </span>
          </div>
