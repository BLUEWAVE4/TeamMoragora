...
export default function DebateCard({ feed, formatTime }) {
  ...
  // ===== 카운트다운 =====
  // HomePage의 enrichVoteDuration이 debate.vote_duration을 채워주므로 여기서 바로 사용
  const voteDuration = debateData?.vote_duration ?? null;
  const createdAt    = debateData?.created_at    ?? null;

  const timeLeft     = useVoteCountdown(createdAt, voteDuration);
  const hasTimer     = !!voteDuration;
  const timerExpired = timeLeft?.expired === true;

  // 화면 표시용 마감 여부
  const isClosed = !isVotingStatus || timerExpired;
  // 실제 투표 가능 여부 (status voting + 타이머 미만료)
    ...
    const fetchSides = async () => {
      try {
        const { data } = await supabase
          .from('debates').select('pro_side, con_side').eq('id', debateId).single();
  ...
  useEffect(() => {
    const debateId = feed?.debate_id || debateData?.id;
    if (!debateId) return;
    getVoteTally(debateId)
      .then(res => setVoteCounts({ agree: res?.A ?? 0, disagree: res?.B ?? 0 }))
      .catch(() => {});
  }, [feed?.debate_id, debateData?.id]);
    ...
    const fetch = async () => {
      try {
        const { count } = await supabase
          .from('debate_likes').select('*', { count: 'exact', head: true }).eq('debate_id', debateId);
        setLikeCount(count ?? 0);
        if (user) {
          const { data } = await supabase
            .from('debate_likes').select('id').eq('debate_id', debateId).eq('user_id', user.id).maybeSingle();
          setLiked(!!data);
        }
  ...
  useEffect(() => {
    ...
    supabase.from('comments').select('*', { count: 'exact', head: true }).eq('debate_id', debateId)
      .then(({ count }) => setLocalCommentCount(count ?? 0))
      .catch(() => {});
  }, [feed?.debate_id, debateData?.id]);
  ...
  const handleVote = async (side) => {
    ...
    if (isVoting || !canVote || isParticipant) return;  // ⭐ canVote 기준
  ...
  const handleLike = async () => {
    ...
    } catch (err) { setLiked(prevLiked); setLikeCount(prevCount); }
  ...
  const handleSendComment = async () => {
    ...
    try {
      const { data, error } = await supabase.from('comments').insert({
        debate_id: debateId, user_id: user.id, content: commentText.trim(),
  ...
  return (
    <>
      <div className="w-full font-sans pb-5 mb-1 bg-white/70 rounded-xl overflow-hidden">
        ...
        <div className="px-4 pb-2 flex items-center gap-1 flex-wrap">
          ...
          {hasTimer && (
            timerExpired ? (
              <span className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-400 font-bold">
                투표 마감
              </span>
            ) : (
              <span>
        ...
        <div className="px-4 pb-4 pt-1">
          <div className="flex flex-col gap-2">
            ...
            <button
              ...
              disabled={isVoting || !canVote || isParticipant}  // ⭐ canVote 사용
              ...
              <div className="relative h-full px-3.5 flex items-center justify-between z-10">
                <div className="flex items-center gap-2">
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
                    ...
                    {isCompleted || timerExpired ? '마감' : 'A측'}
                  </span>
                  <span className={`text-[13px] font-bold ${myVote === 'A' ? 'text-emerald-600' : 'text-[#1B2A4A]/70'}`}>
                    {optionAText || "찬성"}
                  </span>
                </div>
                {(myVote || isCompleted) && (
              ...
              <div className="relative h-full px-3.5 flex items-center justify-between z-10">
                <div className="flex items-center gap-2">
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
                    ...
                    {isCompleted || timerExpired ? '마감' : 'B측'}
                  </span>
                  <span className={`text-[13px] font-bold ${myVote === 'B' ? 'text-red-500' : 'text-[#1B2A4A]/70'}`}>
                    {optionBText || "반대"}
                  </span>
                </div>
                {(myVote || isCompleted) && (
        ...
        <div className="px-4 py-3 flex justify-between items-center">
          <div className="flex items-center gap-5">
            <button onClick={handleLike} disabled={isLiking} className="flex items-center gap-1.5 active:scale-90 transition-transform">
              <svg fill={liked ? '#E63946' : 'none'} stroke={liked ? '#E63946' : '#1B2A4A'} strokeWidth="2" height="18" viewBox="0 0 24 24" width="18" opacity={liked ? 1 : 0.5}>
            ...
            <button onClick={() => setIsCommentOpen(true)} className="flex items-center gap-1.5 active:scale-90 transition-transform">
              <svg fill="none" stroke="#1B2A4A" strokeWidth="2" height="18" viewBox="0 0 24 24" width="18" opacity="0.5">
          ...
          <span className="text-[10px] text-[#1B2A4A]/40 font-bold">
            ...
            <div className="fixed inset-0 z-[201] flex items-end justify-center pointer-events-none">
              ...
              >
                        ...
                        <div key={c.id} className={`flex gap-2.5 ${isMine ? 'flex-row-reverse' : ''}`}>
                          ...
                          <div className={`flex-1 min-w-0 ${isMine ? 'text-right' : ''}`}>
                            <div className={`flex items-center gap-1.5 ${isMine ? 'justify-end' : ''}`}>
                              ...
                              {c.user_id === sideUsers.A && <span className="text-[9px] text-white bg-emerald-500 px-1.5 py-0.5 rounded-full font-bold">A측</span>}
                              {c.user_id === sideUsers.B && <span className="text-[9px] text-white bg-red-500 px-1.5 py-0.5 rounded-full font-bold">B측</span>}
                ...
                <div className="flex-shrink-0 px-4 py-3 border-t border-[#D4AF37]/10 flex items-center gap-2" style={{ paddingBottom: `max(12px, env(safe-area-inset-bottom))` }}>
                  {user && (
                    <div className="w-8 h-8 rounded-full overflow-hidden bg-[#1B2A4A]/10 shrink-0">
                      <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${user.user_metadata?.nickname || user.email || 'me'}`} alt="" className="w-full h-full object-cover" />
                    </div>
                  )}
                  <input
                    ref={commentInputRef}
                  ...
                  <button
                    ...
                    className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 transition-all ${commentText.trim() ? 'bg-[#D4AF37] text-white' : 'bg-[#D4AF37]/20 text-[#D4AF37]/40'}`}
                  >
                    {isSendingComment
