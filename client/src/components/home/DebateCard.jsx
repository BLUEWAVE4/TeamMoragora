...
function DebateCard({ feed }) {
  ...
  return (
    <div className={`w-full max-w-lg mx-auto mb-2 border-b ${isDark ? 'bg-black border-gray-900' : 'bg-white border-gray-100'}`}>
      ...
      <div className="px-4 pb-1">
        <h3 className={`text-[16px] font-semibold mb-3 leading-normal break-keep ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>
          {debateData?.topic}
        </h3>
        
        <div className={`rounded-2xl border p-4 shadow-sm mb-3 ${isDark ? 'bg-[#1a1a1a] border-gray-800' : 'bg-white border-gray-100'}`}>
          <div className="flex items-center justify-between mb-4">
            {isClosed ? (
              ...
              <div className="flex items-center gap-1.5 text-[12px] font-bold" style={{ color: pointColor }}>
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`relative w-full h-12 rounded-xl overflow-hidden transition-all active:scale-[0.98]
                    ${showResult ? 'bg-gray-50' : (isDark ? 'bg-white/5' : 'bg-[#F6F6F6]')}
                ...
                >
                  ...
                  <div className="relative px-4 h-full flex items-center justify-between z-10">
                    <div className="flex items-center gap-2.5">
                      {/* 체크(V) 대신 A, B 원형 아이콘 표시 */}
                      <div 
                        className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black border ${
                          isSelected ? 'bg-[#7B3F3F] text-white border-[#7B3F3F]' : 'border-gray-300 text-gray-400'
                        }`}
                        style={isSelected ? { backgroundColor: pointColor, borderColor: pointColor } : {}}
                      >
                        {option.id}
                      </div>
                      <span className={`text-[14px] ${isSelected ? 'font-bold' : 'font-medium'} ${isSelected ? '' : (isDark ? 'text-gray-300' : 'text-gray-800')}`} 
                    ...
                    {showResult && (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[15px] font-black italic" style={{ color: isSelected ? pointColor : '#999' }}>
                          {option.percent}%
                        </span>
                      </div>
                    )}
      ...
      <div className="px-3 py-2 flex items-center justify-between">
        <div className="flex items-center gap-5">
          <button onClick={handleLike} className="flex items-center gap-1.5 active:scale-90 transition-transform">
            {liked ? (
              <svg color="#ed4956" fill="#ed4956" height="22" viewBox="0 0 48 48" width="22"><path d="M34.6 3.1c-4.5 0-7.9 1.8-10.6 5.6-2.7-3.7-6.1-5.5-10.6-5.5C6 3.1 0 9.6 0 17.6c0 10.1 9.1 18.1 22.5 30.4 0.7 0.7 1.5 1.1 2.5 1.1s1.8-0.4 2.5-1.1c13.4-12.3 22.5-20.3 22.5-30.4 0-8-6-14.5-13.4-14.5z"/></svg>
            ) : (
              <svg color={iconStroke} fill="none" stroke="currentColor" strokeWidth="2" height="22" viewBox="0 0 24 24" width="22"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
            )}
            <span className={`text-[13px] font-bold ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>{likeCount}</span>
          </button>

          <button onClick={() => setIsCommentOpen(true)} className="flex items-center gap-1.5 active:scale-90 transition-transform">
            <svg color={iconStroke} fill="none" stroke="currentColor" strokeWidth="2" height="22" viewBox="0 0 24 24" width="22"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/></svg>
            <span className={`text-[13px] font-bold ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>{localCommentCount}</span>
          </button>

          <div className="flex items-center gap-1.5">
            <svg color={iconStroke} fill="none" stroke="currentColor" strokeWidth="2" height="22" viewBox="0 0 24 24" width="22">
              <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/>
            </svg>
            <span className={`text-[13px] font-bold ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>{viewCount}</span>
          </div>
        </div>
